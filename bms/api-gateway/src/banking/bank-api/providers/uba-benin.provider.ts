...
export default class UbaBeninProvider implements BankProvider {
  private readonly bankCode = 'UBA_BENIN';
  private readonly authUrl = process.env.UBA_BENIN_AUTH_URL;
  private readonly tokenUrl = process.env.UBA_BENIN_TOKEN_URL;
  private readonly apiUrl = process.env.UBA_BENIN_API_BASE_URL;
  private readonly clientId = process.env.UBA_BENIN_CLIENT_ID;
  private readonly clientSecret = process.env.UBA_BENIN_CLIENT_SECRET;
  ...
  async initializeAuth(options: BankAuthOptions): Promise<BankAuthResult> {
    if (this.authUrl && this.clientId) {
      const params = {
        ...
        scope: options.scope || 'accounts transactions profile',
        response_type: 'code',
        state: options.state,
      };
  ...
  async exchangeAuthCode(code: string): Promise<BankTokens> {
    if (this.tokenUrl && this.clientId && this.clientSecret) {
      try {
        ...
        console.warn('[UbaBeninProvider] exchangeAuthCode failed, fallback to stub tokens', error);
      }
    }
  ...
  async refreshAccessToken(refreshToken: string): Promise<BankTokens> {
    if (this.tokenUrl && this.clientId && this.clientSecret) {
      try {
        ...
        console.warn('[UbaBeninProvider] refreshAccessToken failed, fallback to stub tokens', error);
      }
    }
      ...
      try {
        ...
        if (Array.isArray(response.data?.accounts)) {
          return response.data.accounts.map((account: any) => ({
            ...
            iban: account.iban,
            bic: account.bic || 'UNAFBJBJ',
          }));
        }
      } catch (error) {
        console.warn('[UbaBeninProvider] fetchAccounts failed, fallback to stub data', error);
      }
    }

    return [
      {
        id: `${this.bankCode}-bj-current`,
        name: 'Compte courant UBA Bénin',
        type: 'current',
        currency: 'XOF',
        balance: 0,
        iban: undefined,
    ...
    try {
      ...
      if (Array.isArray(response.data?.transactions)) {
        return response.data.transactions.map((tx: any) => ({
          ...
          description: tx.description || tx.label || 'Transaction UBA',
      ...
      console.warn('[UbaBeninProvider] fetchTransactions failed, fallback to stub data', error);
    }
  ...
  async validateAccessToken(accessToken: string): Promise<boolean> {
    ...
    try {
      ...
      console.warn('[UbaBeninProvider] validateAccessToken failed', error);
      return false;
    }
  }
  ...
  async revokeAccess(accessToken: string): Promise<void> {
    ...
    try {
      ...
      console.warn('[UbaBeninProvider] revokeAccess failed', error);
    }
  }
