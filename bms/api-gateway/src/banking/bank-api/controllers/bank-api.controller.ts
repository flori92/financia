...
export class BankApiController {
  ...
  @Post('connections')
  async initializeConnection(@Body() dto: CreateBankConnectionDto) {
    try {
  ...
  async completeAuthentication(
    ...
    try {
      return await this.bankApiService.completeAuthentication(connectionId, code);
    } catch (error) {
      if (error instanceof BankApiException) {
        throw new HttpException(error.message, HttpStatus.BAD_REQUEST);
      }
  ...
  async syncAccounts(@Param('connectionId') connectionId: string) {
    try {
