...
export default function IntelligentAIChatPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '0',
      role: 'assistant',
      content: "👋 Bonjour ! Je suis votre assistant IA intelligent BMS. J'ai accès à vos données comptables en temps réel et je peux vous aider avec :\n\n• 📊 Analyse financière personnalisée\n• 💰 Optimisation fiscale (OHADA/Bénin)\n• 📈 Prévisions et tendances\n• ⚠️ Détection de risques\n• 🎯 Recommandations business\n\nComment puis-je vous aider aujourd'hui ?",
      timestamp: new Date()
    }
  ]);
  ...
  const loadPersonalizedSuggestions = async () => {
    try {
      const response = await apiGet(`/api/v1/ai/suggestions?companyId=${companyId}&userId=${userId}`);
      if (response.success) {
  ...
  const loadStats = async () => {
    try {
      const response = await apiGet(`/api/v1/ai/stats?companyId=${companyId}&userId=${userId}`);
      if (response.success) {
  ...
  const loadHistory = async () => {
    try {
      const response = await apiGet(`/api/v1/ai/history?companyId=${companyId}&userId=${userId}&limit=20`);
      if (response.success) {
  ...
  const handleGenerateReport = async (reportType: string) => {
    ...
    try {
      const response = await apiPost('/api/v1/ai/reports', {
        type: reportType,
        companyId,
        userId
      });
      ...
      <div className="w-80 bg-white border-r border-gray-200 flex flex-col">
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h2 className="font-semibold text-gray-900">Assistant IA BMS</h2>
              <p className="text-sm text-gray-500">Intelligent • Connecté</p>
            </div>
          </div>
          ...
          {stats && (
            <div className="space-y-3">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">{stats.active_days}</span>
              </div>
            </div>
          )}
        </div>
            ...
            {suggestions.map((suggestion, index) => (
              ...
              >
                <div className="flex items-start gap-3">
                  <suggestion.icon className="w-4 h-4 text-blue-600 mt-0.5 flex-shrink-0" />
                  <div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="bg-white border-b border-gray-200 px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-semibold text-gray-900">Chat avec Assistant IA</h1>
              <p className="text-sm text-gray-500">Basé sur GPT-4 avec vos données BMS</p>
            </div>
            <button
        ...
        {showReports && (
          <div className="bg-blue-50 border-b border-blue-200 px-6 py-4">
            <h3 className="font-medium text-blue-900 mb-3">Générer un rapport automatique</h3>
            <div className="grid grid-cols-2 gap-3">
              {REPORT_TYPES.map((report) => (
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-white border border-blue-200 rounded-lg hover:bg-blue-50 transition-colors disabled:opacity-50"
                >
                ...
                <div className={`max-w-3xl ${
                  ...
                  {message.context && (
                    <div className={`mt-3 pt-3 border-t ${
                      message.role === 'user' ? 'border-blue-500' : 'border-gray-100'
                    }`}>
              ...
              <div className="flex gap-4 justify-start">
                ...
                <div className="bg-white border border-gray-200 rounded-2xl rounded-bl-sm px-4 py-3 shadow-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-600">L'IA analyse vos données...</span>
                  </div>
                </div>
              </div>
            )}
